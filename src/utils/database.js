...
export const getCourseIdFromExam = (exam) => {
    if (!db) return null;

    // Find the semester exam name using semester_exam_name_id from the exam
    const semesterExam = db.semesterExamNames.find(sen =>
        sen.id === exam.semester_exam_name_id
    );
    ...
    // Find the course using the exam_name from semesterExam
    const course = db.courses.find(c =>
        c.course_name === semesterExam.exam_name
    );
